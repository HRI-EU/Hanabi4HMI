...
def get_max_fireworks(observation):
    ...
    for label in discarded_cards:
        ...
        if number_in_discard >= num_in_deck_by_rank[rank] and max_fireworks[color] >= rank:
...
class Ruleset:
    ...
    def osawa_discard(observation):
        ...
        for card_index, card in enumerate(observation["card_knowledge"][0]):
            color = card["color"]
            rank = card["rank"]
            if color is not None and fireworks[color] == 5:
                return {"action_type": "DISCARD", "card_index": card_index}
            if color is not None and rank is not None and (rank < fireworks[color] or rank >= max_fireworks[color]):
                return {"action_type": "DISCARD", "card_index": card_index}
            if rank is not None and rank < min(fireworks.values()):
                return {"action_type": "DISCARD", "card_index": card_index}
            ...
            for index, card in enumerate(their_knowledge):
                if card["color"] is None:
                    return {
                        ...
                        "color": their_hand[index]["color"],
                        "target_offset": PLAYER_OFFSET,
                    }
                if card["rank"] is None:
                    return {
                        "action_type": "REVEAL_RANK",
                        "rank": their_hand[index]["rank"],
                        "target_offset": PLAYER_OFFSET,
                    }
        return None
    ...
    def play_safe_card(observation):
        ...
        for card_index, _card in enumerate(observation["card_knowledge"][0]):
    ...
    def play_if_certain(observation):
        ...
        for card_index, card in enumerate(observation["card_knowledge"][0]):
            color = card["color"]
            rank = card["rank"]
            if color is not None and rank is not None and rank == fireworks[color]:
                return {"action_type": "PLAY", "card_index": card_index}
        return None
    ...
    def tell_most_information(observation):
        ...
        if observation["information_tokens"] > 1:
            get_max_fireworks(observation)
            max_affected = -1
            for player_offset in range(1, observation["num_players"]):
                player_hand = observation["observed_hands"][player_offset]
                player_hints = observation["card_knowledge"][player_offset]
                for card, _hint in zip(player_hand, player_hints):
                    affected_colors = 0
                    affected_ranks = 0
